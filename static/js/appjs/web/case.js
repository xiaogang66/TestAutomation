                ...
                columns: [{
                    ...
                    },
					{
						...
						formatter : function(value, row, index) {
							...
							var f = '<a class="btn btn-primary btn-sm '+s_step_h+'" href="#" title="步骤维护"  mce_href="#" onclick="maintainStep(\'' + row.id + '\')">步骤维护</a> ';
							var h = '<a class="btn btn-success btn-sm '+s_executee_h+'" href="#" title="执行"  mce_href="#" onclick="execute(\'' + row.id + '\')">执行</a> ';
							return g + e + d + f + h;
						}
					}]
            }
...
function add() {
	...
	layer.open({
		...
		area : [ '70%', '70%' ],
		content : '/web/caseAddPage?moduleId='+choseModuleId
	});
}
...
function copy(id){
		...
		shadeClose : false, // 点击遮罩关闭层
		area : [ '70%', '70%' ],
		content : '/web/caseCopyPage?id='+ id
	});
}
...
function edit(id) {
	layer.open({
		...
		shadeClose : false, // 点击遮罩关闭层
		area : [ '70%', '70%' ],
		content : '/web/caseEditPage?id='+ id
	});
}
