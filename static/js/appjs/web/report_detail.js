	...
	$('#exampleTable').bootstrapTable(
		{
			...
			url : "/web/reportDetail", // 服务器数据的加载地址
			...
			columns : [
					...
					},
					{
						...
						formatter: function (value, row, index) {
							if(row.assert_type==1){
								return  "值相等"
							}else if(row.assert_type==2){
								return  "值包含"
							}else if(row.assert_type==3){
								return  "值正则"
							}else if(row.assert_type==4){
								return  "文本相等"
							}else if(row.assert_type==5){
								return  "文本包含"
							}else if(row.assert_type==6){
								return  "文本正则"
							}
                        }
					},
					{
						field : 'assert_partern',
						title : '断言表达式'
					},
					{
						field : 'real_result',
