...
def module_tree(request):
    '''返回一级菜单列表'''
    modules = Module.objects.filter(parent_module_number_id = None)
    result = []
    for module in modules:
        result.append({'id':module.id,'text':module.module_number+'_'+module.module_name})
    return HttpResponse(json.dumps(result))


def list_with_page(request):
    return HttpResponse(json.dumps('{"total":0,"rows":null}'))


def module_number_is_exit(request):
...
def module_list(request):
    ...
    if module_number:
        module = module.filter(Q(module_number__contains=module_number))
    if module_type:
        module = module.filter(Q(module_type=module_type))
    if module_name:
        module = module.filter(Q(module_name__contains=module_name))
    # 记录总数
    total = module.count()
    module = module.order_by('id')[int(size) * (int(page) - 1):right_boundary]     #分页切片
    rows = []
    for module in module:
        rows.append({'id': module.id,'module_number':module.module_number, 'module_name': module.module_name, 'module_type': module.module_type,'module_desc': module.module_desc, 'manager':module.manager,'builder':module.builder,'build_time':module.build_time})
    return HttpResponse(json.dumps({'total': total, 'rows': rows},cls=LocalDateEncoder))


def module_add_page(request):
    user_name = request.COOKIES.get('user_name', '').encode('latin-1').decode('utf-8')
    return render(request,'sys/module_add.html',{'user_name':user_name})
